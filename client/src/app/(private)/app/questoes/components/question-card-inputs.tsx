...
export function QuestionCardInputs() {
  ...
  useEffect(() => {
    api.get("/category").then((response) => {
      if (response.data) {
        const mapped = mapCategoriesToLabels(response.data);
        setMappedCategories(mapped);
      }
    });
  }, []);

  const mapCategoriesToLabels = (categories: Category[], depth = 0) => {
    const result: { label: string; value: string }[] = [];

    const traverse = (categories: Category[], currentDepth: number) => {
      categories.forEach((category) => {
        result.push({
          label: `${"--".repeat(currentDepth)} ${category.name}`, // Adiciona "--" conforme a profundidade
          value: result.find((item) => item.value == category.name)
            ? category.name + 1
            : category.name,
        });

        // Se houver categorias filhas, chame a função recursivamente
        if (category.children && category.children.length > 0) {
          traverse(category.children, currentDepth + 1);
        }
      });
    };

    traverse(categories, depth);
    return result;
  };
